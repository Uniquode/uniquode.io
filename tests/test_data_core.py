...
from data_core.models import Base, metadata
...
def test_data_core_package_imports() -> None:
    package = importlib.import_module("data_core")

    assert package.__name__ == "data_core"


def test_data_core_modules_do_not_import_application_or_auth_packages() -> None:
    project_root = Path(__file__).resolve().parents[1]
    forbidden_modules = ("auth_ext", "uniquode")
    data_core_files = sorted((project_root / "src/data_core").rglob("*.py"))

    assert data_core_files
...
def test_data_core_package_is_included_in_build_modules() -> None:
    pyproject = tomllib.loads(
        (Path(__file__).resolve().parents[1] / "pyproject.toml").read_text()
    )

    assert "data_core" in pyproject["tool"]["uv"]["build-backend"]["module-name"]


def test_data_core_models_expose_shared_metadata() -> None:
    assert metadata is Base.metadata


def test_data_core_owns_database_url_helpers(tmp_path: Path) -> None:
...
def test_migration_version_locations_are_discovered_from_configured_modules() -> None:
    version_locations = migration_version_locations_from_modules(("public", "auth_ext"))

    assert len(version_locations) == 1
    assert version_locations[0].as_posix().endswith("auth_ext/migrations/versions")
    assert discover_migration_version_locations("public") == ()
