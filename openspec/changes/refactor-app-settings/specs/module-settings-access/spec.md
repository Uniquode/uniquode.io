...
The system SHALL let each module define and load its own typed settings from the central configuration provider.

#### Scenario: Module loads owned settings
- **WHEN** a module needs typed settings for its own runtime behaviour
- **THEN** it loads those settings through a module-owned loader or accessor using the central configuration provider
