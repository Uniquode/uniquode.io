...
The system SHALL provide static asset infrastructure through a dedicated Wybra static module rather than embedding static-specific behaviour in the web rendering module.

#### Scenario: Static assets are configured
- **WHEN** an application configures static asset serving or collection
- **THEN** Wybra exposes that behaviour through the static asset module boundary
...
- **THEN** Wybra does not mount an app-served static file handler
...
- **THEN** Wybra discovers configured module static sources with deterministic precedence
