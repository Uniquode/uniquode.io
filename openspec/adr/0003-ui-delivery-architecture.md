...
Use `htmx` as the preferred dynamic enhancement layer where partial page updates, inline actions, or progressively enhanced workflows improve the interface.
...
- Whether the first route-manifest export should be JSON, YAML, or support both formats.
- Whether `htmx` should be introduced immediately in the first UI slice or only once base templates and form handling are in place.
...
- What static asset pipeline, if any, is needed beyond baseline template and stylesheet delivery.

## Follow-Up Work

- Define template, layout, and static asset conventions.
- Define the route naming and route-manifest export conventions.
